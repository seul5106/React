...
/** 비동기 처리 함수 구현 */
export const getAllList = createAsyncThunk("ALL/GET_LIST", async (payload, { rejectWithValue }) => {
    let result = null;
...
const allSlice = createSlice({
    ...
    extraReducers: {
        ...
        [getAllList.fulfilled]: (state, { meta, payload }) => {
            ...
            // 지역별 데이터만 추출
            // --> response.data는 ajax를 통해 얻은 json결과
            ...
            cityNames.forEach((v, p) => {
                ...
                // 일주일 치를 반복(i=ajax로 가져온 전체 배열의 index, j는 그래프용으로 생성한 weekState의 index)
                for (let i = weekIndex, j = 0; i < cityItem.confirmed_acc.length; i++, j++) {
                    //'confrimState.날짜' 배열에 cityItem.date[i]와 일치하는 값의 위치를 검색
                    // --> 일치하는 정보가 없다면(=신규로 추가되는 데이터라면?) -1이 반환된다.
